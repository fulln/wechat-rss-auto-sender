...
class Config:
    ...
    SUMMARY_MIN_LENGTH: int = int(os.getenv('SUMMARY_MIN_LENGTH', '150'))  # 增加到150字，支持更丰富的单篇文章总结
    SUMMARY_MAX_LENGTH: int = int(os.getenv('SUMMARY_MAX_LENGTH', '300'))  # 增加到300字，允许更深度的分析
    
    # 发送控制配置
    MAX_ARTICLES_PER_BATCH: int = int(os.getenv('MAX_ARTICLES_PER_BATCH', '3'))  # 每批最多发送文章数
    SEND_INTERVAL_MINUTES: int = int(os.getenv('SEND_INTERVAL_MINUTES', '1'))    # 发送间隔（分钟）
    ...
    SEND_RANDOM_DELAY_MAX: int = int(os.getenv('SEND_RANDOM_DELAY_MAX', '15'))  # 随机延迟最大值（秒）
